...
export default function Home() {
  ...
  return (
    <div className="h-screen bg-[var(--bg)] text-[var(--text-primary)] flex flex-col font-sans selection:bg-[var(--selection)] overflow-hidden transition-colors">
      {/* ─── 1. Header ─── */}
      <MinimalHeader />
      ...
      {errorMessage && (
        <div className="max-w-[850px] mx-auto w-full px-6 pt-4 shrink-0">
          <div className="p-3.5 rounded-md bg-[#FFF5F5] dark:bg-[#2A1515] border border-[#FED7D7] dark:border-[#5A2525] text-xs text-[#C53030] dark:text-[#FC8181] flex items-center justify-between">
        ...
        {isConversationActive ? (
          ...
          <div className="w-full space-y-8 text-left py-4 animate-in fade-in duration-200">
            ...
            <div className="space-y-3.5 max-w-[760px]">
              <span className="text-xs font-mono tracking-widest uppercase text-[var(--text-muted)] block">
                HELLO, I&apos;M SUYASH
              </span>

              <h1 className="text-3xl sm:text-5xl font-bold tracking-tight text-[var(--text-primary)] leading-[1.12]">
                I build software, AI systems, and things I&apos;m curious about.
              </h1>

              <p className="text-sm sm:text-base text-[var(--text-secondary)] leading-relaxed max-w-[620px]">
            ...
            <div className="space-y-3 pt-1">
              <div className="flex items-center">
                <button
                  onClick={handleStartCall}
                  className="w-[220px] h-[48px] inline-flex items-center justify-between px-5 rounded-md bg-[var(--btn-bg)] hover:bg-[var(--btn-hover)] text-[var(--btn-text)] text-sm font-medium transition-all shadow-2xs active:scale-98 cursor-pointer"
                >
                  <div className="flex items-center gap-2.5">
                    <Mic className="w-4 h-4 text-[var(--accent-green)]" />
              ...
              <div className="space-y-1.5 pt-1 max-w-[680px]">
                ...
                <form onSubmit={handleLandingSubmit} className="relative flex items-center">
                  <input
                    ...
                    className="w-full py-3 pl-4 pr-12 text-sm bg-[var(--bg-input)] border border-[var(--line)] rounded-md text-[var(--text-primary)] placeholder-[var(--text-muted)] focus:outline-none focus:border-[var(--text-primary)] transition-colors shadow-2xs font-sans"
                  />
                  <button
                    type="submit"
                    disabled={!landingInput.trim()}
                    className="absolute right-2 p-1.5 rounded-md bg-[var(--btn-bg)] text-[var(--btn-text)] hover:opacity-90 disabled:opacity-30 transition-colors cursor-pointer"
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1.5 text-sm">
                <button
                  ...
                  className="text-left text-[var(--text-primary)] hover:text-[var(--accent-green)] transition-colors flex items-center gap-2 cursor-pointer py-1"
                >
